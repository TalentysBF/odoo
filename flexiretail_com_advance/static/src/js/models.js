odoo.define('flexiretail_com_advance.models', function (require) {
	...
	models.load_fields("res.users", ['image_small','login','company_ids','access_ereceipt','access_quick_cash_payment',
	                                 ...
	                                 'discard_product','based_on','can_give_discount', 'discount_limit','login_with_pos_screen','store_ids']);
	...
	models.load_fields('pos.session',['is_lock_screen','current_cashier_id','locked','locked_by_user_id','opening_balance','store_id']);
	    ...
	    load_server_data: function () {
	        ...
	        return posmodel_super.load_server_data.apply(this, arguments).then(function () {
	        	...
	        	if(self.config.enable_reorder){
					...
					console.log("self.domain_as_args",self.domain_as_args);
					var params = {
						model: 'pos.order',
	    ...
	    _save_to_server: function (orders, options) {
			...
			.done(function(server_ids){
				...
				if(server_ids.length > 0 && self.config.enable_reorder){
					var domain_list;
					if(self.config.multi_store_id && self.config.multi_store_id[0]){
//						domain_list.push(['store_id','=',self.config.multi_store_id[0]])
      ...
      //Rounding
        ...
        export_for_printing: function(){
            ...
            var new_val = {
            	...
            	amount_due: this.get_due() ? this.get_due() : 0.00,
            	old_order_id: this.get_order_id(),
            	reserve_delivery_date: moment(this.get_reserve_delivery_date()).format('L') || false,
            };
            $.extend(orders, new_val);
            return orders;
        },
