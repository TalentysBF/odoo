	...
	ViewManager.include({
		do_execute_action: function (action_data, env, on_closed) {
			...
			if(env.model == 'wizard.pos.x.report' && action_data.id == 'main_print_button'){
	    		...
	    		return self.do_action('flexiretail_com_advance.pos_x_report',{additional_context:{
		...
		switch_mode: function(view_type, view_options) {
	        ...
	        var xml_ids = ['flexiretail_com_advance.action_all_order_kanban_view', 'flexiretail_com_advance.action_all_items_kanban_view']
